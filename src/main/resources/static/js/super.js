...
$(function() {
	...
	$(".panel-body .accordion-body>ul>li").on('click', function() {
		...
		if($("#tt").tabs('exists', tabTitle)) {
		  ...
		  // 计算iframe 高度 ，避免出现纵向滚动条
      var frameHeigth = caculateFrameHeight();
		  // 不能设置url直接引用，否则会造成id污染
      var content = '<iframe scrolling="no" frameborder="0"  src="'+tabUrl+'" style="width:100%;height:'+frameHeigth+'px;"></iframe>';
			$('#tt').tabs('add', {
				title: tabTitle,
        content: content,
        // href:tabUrl,
